...
const palabras = ["JUEGO", "ORACLE", "CISCO", "ALURA", "AHORCADO", "LIBRO", "MATE", "COMIDA", "AMBIENTE", "HISTORIA", "PERSONA", "ETIQUETA", "PROGRAMA", "ESTRELLA", "ANIMAL", "JUSTICIA", "PAZ", "GUERRA", "FRIO", "CALOR", "DERECHO", "LIBERTAD", "TRABAJO", "BLOQUE", "BOLIVIA", "BRILLAR", "CADETE", "CANAL", "BARRIO", "ABEJA", "ASTROS", "CABEZA", "CABRA", "CAJA", "BANCO", "ACTO", "ABRIR", "BEBE", "PALO", "CHILE", "BRASIL", "CONO", "CORRER", "CUBA", "DEDOS", "MANOS", "PIES", "CUERPO", "PIERNAS", "PELO", "CARA", "OJOS", "NARIZ", "DIENTES", "OREJAS", "CEJAS", "LENTES", "TIO", "FABRICA", "PELOTA", "SOPA", "RUSIA", "RELOJ", "TECLADO", "MONITOR", "AUDIO", "TV", "SOL", "CODIGO", "LUNA", "FUEGO", "AGUA", "HIELO", "VIENTO", "PERRO", "GATO"];
...
const mostrarGuiones = (palabra) =>
{
    ...
    for (let i = 0; i < cantidad; i++) 
    {
        ...
        word.innerHTML += `<div class="letter"> <p class="${letra} display-none">${letra}</p> <img src="img/underline.png"> </div>`;
    }
}
...
const mostrarLetraCorrecta = (letra) =>
{
    ...
    letter.forEach(letra => (letra.classList.remove("display-none")));
}
...
const advertir = (frase, colorbg) => 
{
    ...
    setTimeout(() => { adv.style.opacity = "0" }, 2000);
}
...
const verificarFinJuego = (secreto, letrasCorrectas, errores) => 
{
    if (errores.length == 6) 
    {
        ...
        btnAbandonar.classList.add("display-none");
        return true
    }
...
const inputMobile = () => 
{
    ...
    input.addEventListener("keyup", () => { setTimeout(() => { input.value = "" }, 200) });
    return input;
}
